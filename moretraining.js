...
let cilindradaCars = cars.flatMap(({ cc }) => cc).sort((a, b) => a - b);
...
let allNames = cars.map(({ brand }) => brand).sort();
...
let printMinCc = cilindradaCars.find((x) => x > 1);
console.log(printMinCc);
// Other Ways of Max n Min CC

let getTheMaxCc = orderCc.reduce((x, y) => y);
console.log(getTheMaxCc);
let getMacc = Math.max(...orderCc);
console.log(getMacc);

// Other ways to get Min CC =>
let otherMincc = cilindradaCars.shift();
console.log(otherMincc);

// Get all False & True =>
let getAllFalse = cars.filter(({ value }) => value !== true);
console.log(getAllFalse);
let getAllTrue = cars.filter(({ value }) => value !== false);
console.log(getAllTrue);

// Get only false =>
let allFalse = cars.filter((x) => x.value === false);
console.log(allFalse);

let values = cars.map(({ value }) => value).filter((x) => x === true);
console.log(values);
let allFalsos = cars.map(({ value }) => value).filter((x) => x === false);
